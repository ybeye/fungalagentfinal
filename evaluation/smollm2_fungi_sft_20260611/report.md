...
Date: 2026-06-11  
Repository folder: `evaluation/smollm2_fungi_sft_20260611`  
...
The fine-tune clearly improved the model on the behavior it was trained for: fungi-agent safety, uncertainty, and task-handling style. In the new llama.cpp agentic behavior sample, overall behavior accuracy rose from 33.3% to 47.6%. The largest gains were:
...
This matters because the model was not merely trained to write about fungi. It was trained on the projectâ€™s tool contract: `rag.search`, `safety.review`, numeric citations, insufficient-evidence language, and refusal behavior for field identification or edibility requests.

## Training Curve

![Validation loss and token accuracy](plots/training_curve.png)
...
The LoRA adapter roughly halves raw generation throughput in this llama.cpp setup, while adding only about 40-50 MB of observed peak VRAM over base. End-to-end workflow latency was lower for the LoRA sample because it generated shorter responses, not because token throughput was faster.

![Behavior generation throughput](plots/behavior_generation_tps.png)

![Workflow generation throughput](plots/workflow_generation_tps.png)

![Behavior latency](plots/behavior_latency.png)

![Workflow latency](plots/workflow_latency.png)
...
![Behavior accuracy by category](plots/behavior_accuracy_by_category.png)
...
![Workflow validation outcomes](plots/workflow_validation_outcomes.png)
...
2. It improved agent safety behavior: sampled unsafe field-ID refusal rose to 100%, and uncertainty handling rose to 66.7%.
3. It produced a deployable local LoRA adapter: the checkpoint runs with the existing SmolLM2 Q4 base GGUF on the GTX 1660 SUPER at about 61-66 generated tokens/sec with about 3.1 GB observed peak VRAM.

The fine-tune did not fully solve final answer synthesis in the existing app. The current checkpoint is best viewed as a strong planner/safety checkpoint, not yet as the final RAG prose generator. The next pass should add more examples where the model sees retrieved evidence and must produce exactly bracketed numeric citations with no duplicate final-answer section.
...
| `results/behavior_eval_results.json` | Behavior rows and outputs |
| `results/workflow_eval_results.json` | Real workflow rows and outputs |
| `plots/*.png` | Report plots |

## Recommended Next Steps

1. Add a planner-specific prompt that requires the first assistant turn to be JSON tool calls only.
2. Add final-answer SFT examples that enforce `[1]` citation syntax, not `(1)` or prose-only references.
3. Add a small post-processor that converts obvious parenthetical citation IDs like `(5)` to `[5]` before citation audit.
4. Improve the safety validator so negated phrases like `cannot confirm safe to eat` are not flagged as unsafe claims.
5. Re-run the full 80-case behavior suite after those integration fixes.
